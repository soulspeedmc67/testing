...
export default function ProductCard({
  ...
  return (
    <div
      ...
      className="bg-white rounded-2xl border border-slate-200/90 p-2.5 flex flex-col justify-between shadow-2xs hover:shadow-xs transition-all active:scale-[0.99] cursor-pointer group select-none relative"
    >
      {/* Top Image Container */}
      <div className="relative w-full aspect-square bg-slate-50/80 rounded-xl overflow-hidden flex items-center justify-center p-2 border border-slate-100">
        ...
        {/* Discount Tag */}
        {discountPercent && (
          <span className="absolute bottom-1.5 left-1.5 text-[8.5px] font-black text-white bg-[#FF6B00] px-1.5 py-0.5 rounded-md shadow-2xs">
            {discountPercent}% OFF
          </span>
        )}
      </div>
      ...
      <div className="flex items-center justify-between mt-2.5">
        <span className="text-[10px] font-bold text-slate-500 bg-slate-100/90 px-2 py-0.5 rounded-md">
          {product.unit}
        </span>
