...
export default function AnimatedSearchBar({ value, onChange, onFocus, onBlur, isFocused }) {
  ...
  return (
    ...
    >
      <Search className="w-4 h-4 text-slate-400 mr-2.5 shrink-0 dark:text-content-faint" />
      ...
      <div className="relative w-full flex items-center overflow-hidden h-5">
        <input
          ...
          onBlur={onBlur}
          onChange={onChange}
          className="w-full bg-transparent text-xs font-semibold text-slate-900 focus:outline-none z-10 dark:text-content"
        />

        {!value && (
          <span
            key={index}
            className="absolute left-0 text-xs font-semibold text-slate-400 pointer-events-none animate-search-text truncate dark:text-content-faint"
          >
      ...
      <Mic className="w-4 h-4 text-slate-500 ml-2 shrink-0 cursor-pointer hover:text-[#FF5B00] transition-colors dark:text-content-muted" />
    </div>
  );
}
