...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#061838] text-white font-sans flex flex-col justify-between overflow-hidden select-none relative">
      {/* STEP 1: Screenshot-styled Hero + 70% Extensible Bottom Sheet */}
      {step === 1 && (
        <div className="relative min-h-screen flex flex-col justify-between bg-gradient-to-b from-[#061838] via-[#0A2558] to-[#040E22]">
          {/* Top Section: Navigation + Brand Squircle + Punchline + Artwork */}
          <div className="relative z-10 flex-1 flex flex-col px-6 pt-[max(14px,env(safe-area-inset-top,14px))] pb-2 justify-between">
            ...
            <div className="flex items-center justify-between shrink-0">
              <button
                ...
                className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 active:scale-95 flex items-center justify-center text-white backdrop-blur-md transition-colors"
                aria-label="Go back"
              >
              ...
              <button
                ...
                className="text-[11px] font-black text-white/70 hover:text-white bg-white/10 hover:bg-white/20 px-3.5 py-1.5 rounded-full transition-colors active:scale-95"
              >
                Skip for now
              </button>
            </div>

            {/* Centered Squircle Brand Icon (Matching Reference Screenshot) */}
            <div className="text-center pt-2 shrink-0">
              ...
              {/* Bold Punchy Headline (Matching Reference Screenshot) */}
            ...
            {/* Artwork Sitting Underneath Headline (Flush above Bottom Sheet) */}
            <div className="flex-1 flex items-end justify-center pointer-events-none select-none overflow-hidden pb-0">
              {heroFailed ? (
                <img
                  src="/dashit-mark-white.png"
                  alt=""
                  aria-hidden="true"
                  className="w-20 h-20 object-contain opacity-80 mb-4"
                />
              ) : (
                <img
                  ref={heroImgRef}
                  ...
                  className="w-full max-w-[230px] max-h-[25vh] object-contain object-bottom drop-shadow-[0_12px_24px_rgba(0,0,0,0.45)]"
                />
              )}
            </div>
          </div>

          {/* Bottom Card: Collapsed Preview State (Matching Screenshot) */}
          <div className="relative z-20 rounded-t-[32px] bg-white text-slate-900 px-6 pt-5 pb-[max(20px,calc(12px+env(safe-area-inset-bottom,20px)))] shadow-[0_-16px_40px_rgba(0,0,0,0.28)] space-y-3.5 shrink-0">
            {/* Primary Login Button (Expands to 70% Sheet) */}
            <button
              type="button"
              onClick={() => {
                setAuthTab("signup");
              ...
              className="w-full bg-[#FF5B00] hover:bg-[#E04E00] text-white font-black text-[15.5px] py-3.5 rounded-2xl shadow-[0_4px_16px_rgba(255,91,0,0.32)] active:scale-[0.98] transition-all flex items-center justify-center"
            >
              Login
            </button>
            ...
            {/* Secondary Menu List (Offers & Feedback, matching screenshot) */}
            <div className="bg-slate-50 border border-slate-100 rounded-2xl divide-y divide-slate-100 overflow-hidden shadow-xs">
              <button
                type="button"
                onClick={() => router.push("/categories")}
                className="w-full px-4 py-3.5 flex items-center justify-between hover:bg-slate-100/70 transition-colors text-left"
              >
                <div className="flex items-center space-x-3 text-slate-800">
                  <BadgePercent className="w-4 h-4 text-slate-700 stroke-[2.2]" />
                  <span className="text-xs font-extrabold text-slate-800">Offers &amp; Deals</span>
                </div>
              ...
              >
                <div className="flex items-center space-x-3 text-slate-800">
                  ...
                  <span className="text-xs font-extrabold text-slate-800">Feedback &amp; Support</span>
                </div>
                <ChevronRight className="w-4 h-4 text-slate-400" />
              </button>
            </div>
          ...
          {/* ---------------------------------------------------------------- */}
          {/* 70% EXTENDED BOTTOM SHEET WITH NAVIGATION & FULL AUTH FORM        */}
          {/* ---------------------------------------------------------------- */}
          <AnimatePresence>
            {isSheetExtended && (
              <>
                ...
                {/* 70% Height Bottom Sheet Card */}
                <motion.div
                  initial={{ y: "100%" }}
                  animate={{ y: 0 }}
                  exit={{ y: "100%" }}
                  transition={{ type: "spring", damping: 28, stiffness: 280 }}
                  className="fixed inset-x-0 bottom-0 z-50 h-[72vh] bg-white text-slate-900 rounded-t-[32px] shadow-[0_-20px_50px_rgba(0,0,0,0.4)] flex flex-col overflow-hidden"
                >
                  {/* Drag affordance */}
                  <div className="w-10 h-1 rounded-full bg-slate-200 mx-auto mt-2.5 mb-1" />
                  ...
                  <div className="flex items-center justify-between px-5 py-2.5 border-b border-slate-100">
                    <button
                      type="button"
                      onClick={() => setIsSheetExtended(false)}
                      className="flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-slate-900 active:scale-95 transition-colors"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setIsSheetExtended(false)}
                      className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-200 active:scale-90 transition-colors"
                      aria-label="Close"
                    >
                  ...
                  <div className="flex-1 overflow-y-auto overscroll-contain px-6 py-4 space-y-3.5">
                    ...
                    <form onSubmit={handleEmailAuth} className="space-y-3">
                      ...
                      <button
                        ...
                        className="w-full bg-[#FF5B00] hover:bg-[#E04E00] disabled:opacity-60 text-white font-black text-[14px] py-3.5 px-6 rounded-2xl flex items-center justify-center space-x-2 active:scale-[0.98] shadow-[0_4px_14px_rgba(255,91,0,0.25)] transition-all mt-1"
                      >
                        <span>
                          {isProcessing
                            ? "Please wait…"
                    ...
                    <div className="space-y-2 pb-4">
                      ...
                      <button
                        ...
                        disabled={isProcessing}
                        className="w-full bg-white hover:bg-slate-50 active:scale-[0.98] border border-slate-200 text-slate-800 font-extrabold text-[13px] py-3 px-4 rounded-2xl flex items-center justify-center space-x-2.5 transition-all shadow-2xs"
                      >
                      ...
                      <button
                        ...
                        disabled={isProcessing}
                        className="w-full bg-[#0087FF]/10 hover:bg-[#0087FF]/15 active:scale-[0.98] border border-[#0087FF]/30 text-[#0087FF] font-extrabold text-[13px] py-3 px-4 rounded-2xl flex items-center justify-center space-x-2.5 transition-all"
                      >
